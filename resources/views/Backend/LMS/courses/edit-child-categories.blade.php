...
<input class="form-check-input ms-0" type="checkbox" id="cat_id-{{ $child_category->id }}" name="cat_id[]" value="{{ $child_category->id }}" @if(!empty($course->cat_id) && $course->cat_id != "null") @if(in_array($child_category->id, json_decode($course->cat_id))) checked @endif @endif>
